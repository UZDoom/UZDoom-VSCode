...
const nonForcedChalk = new chalk_d.Chalk();
const forcedChalk = new chalk_d.Chalk({ level: 2 });
export function colorize(tokens: Token[], options?: ColorizerOptions): string {
    const colors = options?.colors || {};

    return tokens.reduce((acc, token) => {
        const chalk: chalk_d.ChalkInstance = options?.forceColor ? forcedChalk : nonForcedChalk;
